import {
    ...
    SUPABASE_LOGIN_URL,
    SUPABASE_PKCE_VERIFIER_COOKIE_NAME,
} from "../constants";

export const isLoggedIn = async (): Promise<boolean> =>
    Object.entries(await chrome.storage.sync.get(OPEN_SAUCED_AUTH_TOKEN_KEY))
        .length !== 0;

export const getAuthToken = async (): Promise<string> =>
    (await chrome.storage.sync.get(OPEN_SAUCED_AUTH_TOKEN_KEY))[
        OPEN_SAUCED_AUTH_TOKEN_KEY
    ];
...
export const hasOptedLogOut = async (): Promise<boolean> =>
    (await chrome.storage.local.get(OPEN_SAUCED_OPTED_LOG_OUT_KEY))[
        OPEN_SAUCED_OPTED_LOG_OUT_KEY
    ] === true;

export const removeAuthTokenFromStorage = async (): Promise<void> =>
